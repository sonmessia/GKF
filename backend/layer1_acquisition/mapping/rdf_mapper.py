    ...
    def create_skill_mapping(self, skill_data: Dict) -> URIRef:
        ...
        self.graph.add(
            (
                skill_uri,
                self.gkf.skillName,
                Literal(skill_data["name"], datatype=XSD.string),
            )
        )
        ...
        if "level" in skill_data:
            self.graph.add(
                (
                    skill_uri,
                    self.gkf.skillLevel,
                    Literal(skill_data["level"], datatype=XSD.string),
                )
            )
        ...
        if "description" in skill_data:
            self.graph.add(
                (
                    skill_uri,
                    self.gkf.description,
                    Literal(skill_data["description"], datatype=XSD.string),
                )
            )
    ...
    def create_course_mapping(self, course_data: Dict) -> URIRef:
        ...
        self.graph.add(
            (
                course_uri,
                self.gkf.courseName,
                Literal(course_data["name"], datatype=XSD.string),
            )
        )
        ...
        if "url" in course_data:
            self.graph.add(
                (
                    course_uri,
                    self.gkf.courseURL,
                    Literal(course_data["url"], datatype=XSD.anyURI),
                )
            )
        ...
        if "duration" in course_data:
            self.graph.add(
                (
                    course_uri,
                    self.gkf.duration,
                    Literal(course_data["duration"], datatype=XSD.integer),
                )
            )
        ...
        if "difficulty" in course_data:
            self.graph.add(
                (
                    course_uri,
                    self.gkf.difficulty,
                    Literal(course_data["difficulty"], datatype=XSD.string),
                )
            )
    ...
    def create_job_mapping(self, job_data: Dict) -> URIRef:
        ...
        self.graph.add(
            (
                job_uri,
                self.gkf.jobTitle,
                Literal(job_data["title"], datatype=XSD.string),
            )
        )
        ...
        if "salary" in job_data:
            self.graph.add(
                (
                    job_uri,
                    self.gkf.salary,
                    Literal(job_data["salary"], datatype=XSD.decimal),
                )
            )
        ...
        if "description" in job_data:
            self.graph.add(
                (
                    job_uri,
                    self.gkf.description,
                    Literal(job_data["description"], datatype=XSD.string),
                )
            )
